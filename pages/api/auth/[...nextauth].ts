    ...
    providers: [
      ...
      CredentialsProvider({
        ...
        async authorize(credentials, req) {
          ...
          const result = await fetch(`${process.env.NEXTAUTH_URL}/api/loginApi`, {
            method: "POST",
    ...
    callbacks: {
      async jwt({ token, account, user, profile }) {
        ...
        if (String(req.url).includes("session?q=update")) {
          const { placeId, picture }: { placeId: string; picture: string } =
            req.query
...
const NextAuthWrapper = async (req: NextApiRequest, res: NextApiResponse) => {
  const options: NextAuthOptions = authOptions(req)
